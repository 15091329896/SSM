...
use tmptest;
drop procedure if exists procedure_test_1;
delimiter $$
	create procedure procedure_test_1(in num1 int, in num2 int, out res int)
		begin
			set res = num1 + num2;
		end
	$$
...
set @num1 = 12;
set @num2 = 12;
call procedure_test_1(@num1, @num2, @res);
select @res;

declare ref_cur cursor for select *  from people where id = 1;

drop procedure if exists procedure_test_2;
delimiter $$
	create procedure procedure_test_2
	(
		in beginIndex int,
		in endIndex int, 
		out ref_cur cursor 
